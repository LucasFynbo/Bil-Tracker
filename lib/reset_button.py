...
class ResetButton:
    ...
    def monitor_button(self):
        ...
        while True:
            if self.reset_button.value() == 0:  # Button is pressed
                press_duration += 1  # Increment duration every second
                ...
                time.sleep(1)  # Wait for a second before next check
            else:
                if press_duration >= 10 and press_duration < 30:
                    self.perform_reset(level=1)  # Perform level 1 reset if released between 10 and 30 seconds

                self.led.off()  # Turn off LED
                press_duration = 0  # Reset duration counter
